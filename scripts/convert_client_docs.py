...
def process_table(doc, lines, start_index):
    ...
    while i < len(lines) and lines[i].strip().startswith('|'):
        line = lines[i].strip()
        # Skip separator rows (contains only | - characters)
        if not re.match(r'^\|[\s\-\|:]+\|$', line):
            cells = [clean_markdown(cell.strip()) for cell in line.split('|')[1:-1]]
            if cells and any(cell for cell in cells):  # Skip if all cells empty
...
def main():
    ...
    documents = [
        ...
        ('PILOT_IMPLEMENTATION_PLAN.md', 'PILOT_IMPLEMENTATION_PLAN.docx'),
    ]
